import electron, { Tray, Menu, app as app$1, BrowserWindow, ipcMain as ipcMain$1 } from "electron";
...
function createTray(mainWindow2) {
  try {
    ...
    const contextMenu = Menu.buildFromTemplate([
      {
        ...
        click: () => {
          mainWindow2.webContents.send("poller:forceUpdate");
        }
      },
      {
        label: "Configuración",
    ...
    console.error("Error creating tray:", error2);
    return null;
  }
}
...
class BaseProvider {
  ...
  color;
  icon;
  constructor(id2, name, color, icon) {
    this.id = id2;
    this.name = name;
    this.color = color;
    this.icon = icon;
...
class OpenRouterProvider extends BaseProvider {
  ...
  async fetch(apiKey) {
    try {
      ...
      console.error("OpenRouter fetch error:", error2);
      return {
        used: null,
        limit: null,
        unit: "usd",
  ...
  async fetch(apiKey) {
    try {
      ...
      if (usageResponse.ok) {
        ...
        totalUsage = usageData.total_usage / 100 || 0;
      }
      return {
        used: totalUsage,
        limit: softLimit,
      ...
      console.error("OpenAI fetch error:", error2);
      return {
        used: null,
        limit: null,
        unit: "usd",
...
class AnthropicProvider extends BaseProvider {
  ...
  async fetch(apiKey) {
    try {
      ...
      const localData = store$2.get("anthropic-session-tokens", {
        used: 0,
        limit: 0
      });
      return {
      ...
      console.error("Anthropic fetch error:", error2);
      return {
        used: null,
        limit: null,
        unit: "tokens",
...
class XAIProvider extends BaseProvider {
  ...
  async fetch(apiKey) {
    try {
      ...
      console.error("xAI fetch error:", error2);
      return {
        used: null,
        limit: null,
        unit: "usd",
...
class GeminiProvider extends BaseProvider {
  ...
  async fetch(apiKey) {
    try {
      ...
      console.error("Gemini fetch error:", error2);
      return {
        used: null,
        limit: null,
        unit: "tokens",
...
function initPoller() {
  ...
  ipcMain$1.handle("poller:start", () => {
    if (!pollerInterval) {
      startPoller();
    }
  });
  ipcMain$1.handle("poller:stop", () => {
    stopPoller();
  });
  ipcMain$1.handle("poller:forceUpdate", () => {
    pollProviders();
  });
  ipcMain$1.handle("provider:getData", () => {
    return store$1.get("providerData", {});
  });
  startPoller();
}
...
async function pollProviders() {
  ...
  const providerData = {};
  const apiKeys = store$1.get("apiKeys", {});
  for (const [provId, provider] of providers) {
    ...
    try {
      ...
      console.error(`Error fetching ${provId}:`, error2);
      providerData[provId] = {
        status: "error",
        used: null,
        limit: null,
  ...
  store$1.set("providerData", providerData);
  store$1.set("lastUpdate", timestamp);
...
function setupIpcHandlers() {
  ipcMain$1.handle("store:get", (_event, key, defaultValue) => {
  ...
  ipcMain$1.on("navigate:settings", () => {
